...
# --- UI OVERLAY ---
def labels():
    global fps_label, trigger_label, assist_label, silent_label, fov_label
    root = tk.Tk()
    ...
    trigger_label.master.geometry("+14+56")
    setup_stealth(trigger_label.master)
    trigger_label.pack()

    assist_label = tk.Label(text="Aim Assist: Unactive", font=('Tahoma','10'), fg='red', bg='black')
    assist_label.master.geometry("+14+76")
    setup_stealth(assist_label.master)
    assist_label.pack()

    silent_label = tk.Label(text="Silent Aim: Unactive", font=('Tahoma','10'), fg='red', bg='black')
    silent_label.master.geometry("+14+96")
    setup_stealth(silent_label.master)
    silent_label.pack()
...
# --- MAIN LOOP ---
def main():
    global aim_assist, triggerbot, silent_aim, activation_range, is_running
    ...
    print("System Ready.")
    print("Hotkeys: [ ` ] = Aim | [ ALT ] = Trigger | [ P ] = Silent Aim | [ UP/DOWN ] = FOV")
        ...
        while True:
            ...
            if keyboard.is_pressed('alt'):
                if triggerbot_toggle[0]:
                    ...
                    trigger_label.config(text=f"Triggerbot: {'Active' if triggerbot else 'Unactive'}", fg='green' if triggerbot else 'red')
                    triggerbot_toggle[0] = False
                    threading.Thread(target=cooldown, args=(triggerbot_toggle, 0.2)).start()

            if keyboard.is_pressed('`'):
                if aim_assist_toggle[0]:
                    aim_assist = not aim_assist
                    assist_label.config(text=f"Aim Assist: {'Active' if aim_assist else 'Unactive'}", fg='green' if aim_assist else 'red')
                    aim_assist_toggle[0] = False
                    threading.Thread(target=cooldown, args=(aim_assist_toggle, 0.2)).start()

            if keyboard.is_pressed('p'):
                if silent_toggle[0]:
                    silent_aim = not silent_aim
                    silent_label.config(text=f"Silent Aim: {'Active' if silent_aim else 'Unactive'}", fg='green' if silent_aim else 'red')
                    silent_toggle[0] = False
                    threading.Thread(target=cooldown, args=(silent_toggle, 0.2)).start()
            ...
            if closest_part != -1:
                ...
                # 2. Silent Aim (Raw snap)
                if silent_aim and silent_aim_not_cooldown[0]:
                    ...
                    threading.Thread(target=cooldown, args=(silent_aim_not_cooldown, 0.2)).start()

                # 3. Aim Assist (Requires Left Mouse Button hold: 0x01)
                if aim_assist and closest_part_distance < activation_range and send_next[0] and win32api.GetAsyncKeyState(0x01) & 0x8000 > 0:
                    xdif = (head_center_list[0] - screenshot_center[0]) * AIM_SPEED * target_multiply[MONITOR_SCALE]
                    ydif = (head_center_list[1] - screenshot_center[1]) * AIM_SPEED * target_multiply[MONITOR_SCALE]
                    
                    # Smoothing Logic
                    xdif *= 0.2 if abs(xdif) > 20 else 0.4
                    ydif *= 0.2 if abs(ydif) > 20 else 0.4
